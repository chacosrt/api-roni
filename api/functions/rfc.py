...
import functions.common as _common
...
def compact(number):
    ...
    return _common.clean(number, "-_ ").upper().strip()
...
def _get_date(number):
    ...
    day = int(number[4:6])
    try:
        return _dt.date(year + 2000, month, day)
    except ValueError:
        raise _common.InvalidComponent()
...
def calc_check_digit(number):
    ...
    number = _common.to_unicode(number)
    number = ("   " + number)[-12:]
...
def validate(number, validate_check_digits=False):
    ...
    n = _common.to_unicode(number)
    if len(n) in (10, 13):
        # number assigned to person
        if not _re.match("^[A-Z&Ñ]{4}[0-9]{6}[0-9A-Z]{0,3}$", n):
            raise _common.InvalidFormat()
        if n[:4] in _name_blacklist:
            raise _common.InvalidComponent()
        _get_date(n[4:10])
    elif len(n) == 12:
        # number assigned to company
        if not _re.match("^[A-Z&Ñ]{3}[0-9]{6}[0-9A-Z]{3}$", n):
            raise _common.InvalidFormat()
        _get_date(n[3:9])
    else:
        raise _common.InvalidLength()
    if validate_check_digits and len(n) >= 12:
        if not _re.match("^[1-9A-V][1-9A-Z][0-9A]$", n[-3:]):
            raise _common.InvalidComponent()
        if n[-1] != calc_check_digit(n[:-1]):
            raise _common.InvalidChecksum()
    return number
